...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    Vector3f edge1 = this ->v1 - this ->v0;
    Vector3f edge2 = this ->v2 - this ->v0;
    Vector3f s = ray.origin - this->v0;
    Vector3f s1 = crossProduct(ray.direction, edge2);
    Vector3f s2 = crossProduct(s, edge1);
    float det = dotProduct(edge1, s1);

    if (det > -EPSILON && det < EPSILON){
        inter.happened = false;    // This ray is parallel to this triangle.
        return inter;
    }

    float inv_det = 1.0 / det;
    float u = inv_det * dotProduct(s, s1);
    //u, v and 1-u-v are the coordinate under barycentric coordinates
    //if the ray intersects the triangle, u and v and 1 - u -v should range from 0-1
    if (u < 0 || u > 1){// check the first coordinate
        inter.happened = false;
        return inter; 
    }
    float v = inv_det * dotProduct(ray.direction, s2);

    if (v < 0 || u + v > 1){//check the other 2 coordinate
        inter.happened = false;
        return inter;
    }

    // At this stage we can compute t to find out where the intersection point is on the line.
    float t = inv_det * dotProduct(edge2, s2);

    if(t > EPSILON){//the ray intersects with this triangle
