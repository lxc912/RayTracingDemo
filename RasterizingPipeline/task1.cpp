...
int main(int argc, const char **argv) {
    ...
    // load house mesh
    Mesh house_mesh;
    if (!house_mesh.load_obj("../models/house.obj")) {
        std::cerr << "Failed to load house mesh." << std::endl;
        return -1;
    }

    // manually assign some colors for the house
    // roof {240, 70, 80}
    // base {73, 182, 109}
    // chimney {93, 100, 38}
    std::vector<Eigen::Vector3f> cols;
    for (size_t i = 0; i < 6; i++) {
        cols.push_back({240, 70, 80});
    }
    for (size_t i = 0; i < 8; i++) {
        cols.push_back({73, 182, 109});
    }
    for (size_t i = 0; i < 8; i++) {
        cols.push_back({93, 100, 38});
    }
    ...
    if (command_line) {
        ...
        r.draw(pos_id, ind_id, col_id, rst::Primitive::Triangle, culling, anti_aliasing);
